...
class ResNet34(nn.Module):
    def __init__(self, img_size, img_channel, n_classes) -> None:
        ...
        res_block9_out_channels = 256
        self.features.extend(ResidualBlock(in_channels=res_block8_out_channels,
                                           out_channels=res_block9_out_channels,
                                           stride=1).get_block())

        # 1 residual block with kernel size 3x3, 256 filters, stride 1, padding 1
        res_block10_out_channels = 256
        self.features.extend(ResidualBlock(in_channels=res_block9_out_channels,
                                           out_channels=res_block10_out_channels,
                                           stride=1).get_block())

        # 1 residual block with kernel size 3x3, 256 filters, stride 1, padding 1
        res_block11_out_channels = 256
        self.features.extend(ResidualBlock(in_channels=res_block10_out_channels,
                                           out_channels=res_block11_out_channels,
                                           stride=1).get_block())

        # 1 residual block with kernel size 3x3, 256 filters, stride 1, padding 1
        res_block12_out_channels = 256
        self.features.extend(ResidualBlock(in_channels=res_block11_out_channels,
                                           out_channels=res_block12_out_channels,
                                           stride=1).get_block())

        # 1 residual block with kernel size 3x3, 256 filters, stride 1, padding 1
        res_block13_out_channels = 256
        self.features.extend(ResidualBlock(in_channels=res_block12_out_channels,
                                           out_channels=res_block13_out_channels,
                                           stride=1).get_block())

        # 1 residual block with kernel size 3x3, 256 filters, stride 2, padding 1
        res_block14_out_channels = 512
        self.features.extend(ResidualBlock(in_channels=res_block13_out_channels,
                                           out_channels=res_block14_out_channels,
                                           stride=2).get_block())
        img_size = (img_size - 3) // 2 + 1
        
        # 1 residual block with kernel size 3x3, 256 filters, stride 1, padding 1
        res_block15_out_channels = 512
        self.features.extend(ResidualBlock(in_channels=res_block14_out_channels,
                                           out_channels=res_block15_out_channels,
                                           stride=1).get_block())
        
        # 1 residual block with kernel size 3x3, 256 filters, stride 2, padding 1
        res_block16_out_channels = 512
        self.features.extend(ResidualBlock(in_channels=res_block15_out_channels,
                                           out_channels=res_block16_out_channels,
                                           stride=1).get_block())
